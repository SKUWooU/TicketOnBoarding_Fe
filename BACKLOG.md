...
| 2        | 좌석 조회가 `reserved`만 사용하고 점유·만료 상태를 표현하지 않는다 | Backend 응답 fixture, AVAILABLE·HELD·RESERVED 렌더링 테스트 | WebSocket·실시간 push                   | 진행 중 ([#8](https://github.com/SKUWooU/TicketOnBoarding_Fe/issues/8)) |
...
`✨ [FEAT] 좌석 선택의 서버 임시 점유·해제 연동`

- 선택 좌석의 점유 POST와 선택 해제·페이지 이탈 시 DELETE 계약
- 동시 선택 409와 인증 401 사용자 피드백
- 점유 만료 후 서버 재조회와 결제 이동 전 소유권 유지
- 외부 결제·지도·실제 Backend 호출 없이 fixture 검증
